package day_35;

import java.sql.*;
import java.util.*;

class EmployeePayroll {
    int id;
    String name;
    double salary;
    String gender;
    String startDate;
    List<String> departments;

    EmployeePayroll(int id, String name, double salary, String gender, String startDate, List<String> departments) {
        this.id = id;
        this.name = name;
        this.salary = salary;
        this.gender = gender;
        this.startDate = startDate;
        this.departments = departments;
    }
}

public class uc10_PayrollService {
    private static final String URL = "jdbc:mysql://localhost:3306/payroll_service";
    private static final String USER = "root";
    private static final String PASS = "password";

    public boolean addEmployee(EmployeePayroll emp) {
        Connection conn = null;
        try {
            conn = DriverManager.getConnection(URL, USER, PASS);
            conn.setAutoCommit(false);

            String sql1 = "INSERT INTO employee_payroll (id, name, salary, gender, start_date) VALUES (?, ?, ?, ?, ?)";
            PreparedStatement stmt1 = conn.prepareStatement(sql1);
            stmt1.setInt(1, emp.id);
            stmt1.setString(2, emp.name);
            stmt1.setDouble(3, emp.salary);
            stmt1.setString(4, emp.gender);
            stmt1.setString(5, emp.startDate);
            int rows1 = stmt1.executeUpdate();

            double deduction = 0.2 * emp.salary;
            double taxablePay = emp.salary - deduction;
            double tax = 0.1 * taxablePay;
            double netPay = emp.salary - tax;

            String sql2 = "INSERT INTO payroll_details (emp_id, deduction, taxable_pay, tax, net_pay) VALUES (?, ?, ?, ?, ?)";
            PreparedStatement stmt2 = conn.prepareStatement(sql2);
            stmt2.setInt(1, emp.id);
            stmt2.setDouble(2, deduction);
            stmt2.setDouble(3, taxablePay);
            stmt2.setDouble(4, tax);
            stmt2.setDouble(5, netPay);
            int rows2 = stmt2.executeUpdate();

            int deptRows = 0;
            for (String deptName : emp.departments) {
                String sqlDept = "INSERT INTO department (dept_name) VALUES (?) ON DUPLICATE KEY UPDATE dept_name=dept_name";
                PreparedStatement stmtDept = conn.prepareStatement(sqlDept, Statement.RETURN_GENERATED_KEYS);
                stmtDept.setString(1, deptName);
                stmtDept.executeUpdate();

                int deptId;
                ResultSet rs = stmtDept.getGeneratedKeys();
                if (rs.next()) {
                    deptId = rs.getInt(1);
                } else {
                    PreparedStatement stmtGet = conn.prepareStatement("SELECT dept_id FROM department WHERE dept_name=?");
                    stmtGet.setString(1, deptName);
                    ResultSet rs2 = stmtGet.executeQuery();
                    rs2.next();
                    deptId = rs2.getInt("dept_id");
                }

                String sqlEmpDept = "INSERT INTO employee_department (emp_id, dept_id) VALUES (?, ?)";
                PreparedStatement stmtEmpDept = conn.prepareStatement(sqlEmpDept);
                stmtEmpDept.setInt(1, emp.id);
                stmtEmpDept.setInt(2, deptId);
                deptRows += stmtEmpDept.executeUpdate();
            }

            if (rows1 == 1 && rows2 == 1 && deptRows == emp.departments.size()) {
                conn.commit();
                return true;
            } else {
                conn.rollback();
                return false;
            }
        } catch (Exception e) {
            e.printStackTrace();
            try { if (conn != null) conn.rollback(); } catch (Exception ex) {}
            return false;
        } finally {
            try { if (conn != null) conn.close(); } catch (Exception e) {}
        }
    }

    // UC3: Retrieve All Employees
    public void getAllEmployees() {
        try (Connection conn = DriverManager.getConnection(URL, USER, PASS)) {
            String sql = "SELECT e.id, e.name, e.salary, p.net_pay, d.dept_name " +
                         "FROM employee_payroll e " +
                         "JOIN payroll_details p ON e.id = p.emp_id " +
                         "JOIN employee_department ed ON e.id = ed.emp_id " +
                         "JOIN department d ON ed.dept_id = d.dept_id";
            ResultSet rs = conn.createStatement().executeQuery(sql);
            while (rs.next()) {
                System.out.println(rs.getInt("id") + " | " + rs.getString("name") +
                                   " | Salary: " + rs.getDouble("salary") +
                                   " | NetPay: " + rs.getDouble("net_pay") +
                                   " | Dept: " + rs.getString("dept_name"));
            }
        } catch (Exception e) { e.printStackTrace(); }
    }

    // UC4: Update Salary
    public void updateSalary(int empId, double newSalary) {
        try (Connection conn = DriverManager.getConnection(URL, USER, PASS)) {
            conn.setAutoCommit(false);

            PreparedStatement stmt = conn.prepareStatement("UPDATE employee_payroll SET salary=? WHERE id=?");
            stmt.setDouble(1, newSalary);
            stmt.setInt(2, empId);
            stmt.executeUpdate();

            double deduction = 0.2 * newSalary;
            double taxablePay = newSalary - deduction;
            double tax = 0.1 * taxablePay;
            double netPay = newSalary - tax;

            PreparedStatement stmt2 = conn.prepareStatement("UPDATE payroll_details SET deduction=?, taxable_pay=?, tax=?, net_pay=? WHERE emp_id=?");
            stmt2.setDouble(1, deduction);
            stmt2.setDouble(2, taxablePay);
            stmt2.setDouble(3, tax);
            stmt2.setDouble(4, netPay);
            stmt2.setInt(5, empId);
            stmt2.executeUpdate();

            conn.commit();
        } catch (Exception e) { e.printStackTrace(); }
    }

    // UC5: Retrieve Employees by Date Range
    public void getEmployeesByDateRange(String start, String end) {
        try (Connection conn = DriverManager.getConnection(URL, USER, PASS)) {
            String sql = "SELECT * FROM employee_payroll WHERE start_date BETWEEN ? AND ?";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setString(1, start);
            stmt.setString(2, end);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                System.out.println(rs.getInt("id") + " | " + rs.getString("name") + " | " + rs.getString("start_date"));
            }
        } catch (Exception e) { e.printStackTrace(); }
    }

    // UC6: Aggregate Functions
    public void getAverageSalaryByGender() {
        try (Connection conn = DriverManager.getConnection(URL, USER, PASS)) {
            String sql = "SELECT gender, AVG(salary) AS avg_salary FROM employee_payroll GROUP BY gender";
            ResultSet rs = conn.createStatement().executeQuery(sql);
            while (rs.next()) {
                System.out.println(rs.getString("gender") + " | Avg Salary: " + rs.getDouble("avg_salary"));
            }
        } catch (Exception e) { e.printStackTrace(); }
    }

    // UC7: Delete Employee (Cascade delete handles payroll_details + employee_department)
    public void deleteEmployee(int empId) {
        try (Connection conn = DriverManager.getConnection(URL, USER, PASS)) {
            PreparedStatement stmt = conn.prepareStatement("DELETE FROM employee_payroll WHERE id=?");
            stmt.setInt(1, empId);
            stmt.executeUpdate();
            System.out.println("Employee " + empId + " deleted.");
        } catch (Exception e) { e.printStackTrace(); }
    }


    public static void main(String[] args) {
        PayrollService service = new PayrollService();

        EmployeePayroll emp = new EmployeePayroll(1, "Vimalesh", 50000, "M", "2026-01-09", Arrays.asList("HR", "Finance"));
        service.addEmployee(emp);

        service.getAllEmployees();

        service.updateSalary(1, 60000);

        service.getEmployeesByDateRange("2025-01-01", "2026-12-31");

        service.getAverageSalaryByGender();

        service.deleteEmployee(1);
    }
}
